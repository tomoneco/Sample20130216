...
@implementation BezierMenuLayer
...
+(CCScene *) scene
{
	...
	BezierMenuLayer *layer = [BezierMenuLayer node];
...
-(id) init
{
	...
	if( (self=[super init]) ) {
        ...
        CCMenuItemLabel *itemLabel1 = [CCMenuItemLabel itemWithLabel:label1 block:^(id sender) {
            [[CCDirector sharedDirector] pushScene:[BezierByTestLayer scene]];
        }];
		
        CCLabelTTF *label2 = [CCLabelTTF labelWithString:@"CCBezierTo" fontName:@"Marker Felt" fontSize:30];
        CCMenuItemLabel *itemLabel2 = [CCMenuItemLabel itemWithLabel:label2 block:^(id sender) {
            [[CCDirector sharedDirector] pushScene:[BezierToTestLayer scene]];
        }];
