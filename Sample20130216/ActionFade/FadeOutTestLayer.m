...
@implementation FadeOutTestLayer
...
+(CCScene *) scene
{
	...
	FadeOutTestLayer *layer = [FadeOutTestLayer node];
...
-(id) init
{
	...
	if( (self=[super init]) ) {
        ...
        CCLabelTTF *label1 = [CCLabelTTF labelWithString:@"CCFadeOut" fontName:@"Marker Felt" fontSize:30];
        CCMenuItemLabel *itemLabel1 = [CCMenuItemLabel itemWithLabel:label1 block:^(id sender) {
            // 今いるpositionから1.0秒かけて、x座標に110、y座標に320移動
            sprite.position = center;
            [sprite runAction:[CCFadeOut actionWithDuration:3.0f]];
        }];
