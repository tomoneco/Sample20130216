...
#import "JumpByTestLayer.h"
#import "JumpToTestLayer.h"

@implementation JumpMenuLayer
...
+(CCScene *) scene
{
	...
	JumpMenuLayer *layer = [JumpMenuLayer node];
...
-(id) init
{
	...
	if( (self=[super init]) ) {
        ...
        CCMenuItemLabel *itemLabel1 = [CCMenuItemLabel itemWithLabel:label1 block:^(id sender) {
            [[CCDirector sharedDirector] pushScene:[JumpByTestLayer scene]];
        }];
		
        CCLabelTTF *label2 = [CCLabelTTF labelWithString:@"CCJumpTo" fontName:@"Marker Felt" fontSize:30];
        CCMenuItemLabel *itemLabel2 = [CCMenuItemLabel itemWithLabel:label2 block:^(id sender) {
            [[CCDirector sharedDirector] pushScene:[JumpToTestLayer scene]];
        }];
