...
#import "MoveByTestLayer.h"
#import "MoveToTestLayer.h"

@implementation MoveMenuLayer
...
+(CCScene *) scene
{
	...
	MoveMenuLayer *layer = [MoveMenuLayer node];
...
-(id) init
{
	...
	if( (self=[super init]) ) {
        ...
        CCMenuItemLabel *itemLabel1 = [CCMenuItemLabel itemWithLabel:label1 block:^(id sender) {
            [[CCDirector sharedDirector] pushScene:[MoveByTestLayer scene]];
        }];
		
        CCLabelTTF *label2 = [CCLabelTTF labelWithString:@"CCMoveTo" fontName:@"Marker Felt" fontSize:30];
        CCMenuItemLabel *itemLabel2 = [CCMenuItemLabel itemWithLabel:label2 block:^(id sender) {
            [[CCDirector sharedDirector] pushScene:[MoveToTestLayer scene]];
        }];
