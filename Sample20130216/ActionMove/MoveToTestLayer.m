...
@implementation MoveToTestLayer
...
+(CCScene *) scene
{
	...
	MoveToTestLayer *layer = [MoveToTestLayer node];
...
-(id) init
{
	...
	if( (self=[super init]) ) {
        ...
        CCLabelTTF *label1 = [CCLabelTTF labelWithString:@"CCMoveTo" fontName:@"Marker Felt" fontSize:30];
        CCMenuItemLabel *itemLabel1 = [CCMenuItemLabel itemWithLabel:label1 block:^(id sender) {
            // 今いるpositionから1.0秒かけて、x座標に110、y座標に320移動
            sprite.position = center;
            [sprite runAction:[CCMoveTo actionWithDuration:1.0f position:ccp(110, 320)]];
        }];
