...
@implementation RotateToTestLayer
...
+(CCScene *) scene
{
	...
	RotateToTestLayer *layer = [RotateToTestLayer node];
...
-(id) init
{
	...
	if( (self=[super init]) ) {
        ...
        CCLabelTTF *label1 = [CCLabelTTF labelWithString:@"CCRotateTo" fontName:@"Marker Felt" fontSize:30];
        CCMenuItemLabel *itemLabel1 = [CCMenuItemLabel itemWithLabel:label1 block:^(id sender) {
            // 今いる
            sprite.rotation = 0.0f;
            [sprite runAction:[CCRotateTo actionWithDuration:1.0f angle:180.0]];
        }];
