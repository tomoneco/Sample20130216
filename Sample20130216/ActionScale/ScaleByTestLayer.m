...
@implementation ScaleByTestLayer
...
+(CCScene *) scene
{
	...
	ScaleByTestLayer *layer = [ScaleByTestLayer node];
...
-(id) init
{
	...
	if( (self=[super init]) ) {
        ...
        CCMenuItemLabel *itemLabel1 = [CCMenuItemLabel itemWithLabel:label1 block:^(id sender) {
            // タップするたびに、画像が1.25倍になります
            [sprite runAction:[CCScaleBy actionWithDuration:1.0f scale:1.25f]];
        }];
